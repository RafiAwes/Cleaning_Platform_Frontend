...
const SERVICES_STORAGE_KEY = "package_services_edit";
...
export default function PackageEdit() {
  ...
  const handleSubmit = async (values: FieldValues) => {
    try {
      ...
      const result = await updatePackage({ id: packageId, ...packageData }).unwrap();
      ...
      toast.error(error?.data?.message || error?.message || "Failed to update package");
    }
  };
                  ...
                  {selectedAddons.map((item: any) => (
                    <div key={item.id} className="flex items-center space-x-2">
                      ...
                      <Input
                        ...
                        value={item.price}
                        onChange={(e) =>
                          updateAddonPrice(item.id, Number(e.target.value))
                        }
                      />
                      ...
                      <DeleteBtn
                        className="size-12"
                        onClick={() => removeSelectedAddon(item.id)}
                      />
                    </div>
                  ))}
                </div>
